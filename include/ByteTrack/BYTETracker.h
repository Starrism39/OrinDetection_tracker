...
public:
    GMC(int downscale);
    Eigen::Matrix<float, 2, 3, Eigen::RowMajor> applySparseOptFlow(cv::Mat img);
private:
    int downscale_;
...
public:
    ...
    BYTETracker(const int& frame_rate = 30,
                ...
                const float& high_thresh = 0.4,
                const float& match_thresh = 0.8,
                const int distance_mode =0 );  ///// '0':iou distance  '1' feature distance
    ~BYTETracker();

    std::vector<STrackPtr> update(const std::vector<Object>& objects,cv::Mat* img);
...
private:
    ...
    std::vector<std::vector<float> > calcfeatureDistance(const std::vector<STrackPtr> &a_tracks,
                                                         const std::vector<STrackPtr> &b_tracks) const;

    std::vector<std::vector<float>> calcIous(const std::vector<Rect<float>> &a_rect,
                                             const std::vector<Rect<float>> &b_rect) const;
    
    std::vector<std::vector<float>> calcdistance(const std::vector<std::vector<float>> &a_rect,
